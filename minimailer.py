...
def send_email(conn: smtplib.SMTP, mail: EmailMessage):
    try:
        ...
        print(f"Failed to send mail to {mail['To']}! Reason: {exp}", file=sys.stderr)
...
def compose_mass_email(
    data: list[dict[str, str]],
    mail_headers: dict[str, str],
    recipients_format: list[str],
    template: Template,
    attachments_formats: Optional[list[str]] = None,
) -> Generator[EmailMessage, None, None]:
    for line in data:
        context = defaultdict(str, line)
        # pylint: disable=cell-var-from-loop
        mail_headers["To"] = ", ".join(
            map(lambda s: s.format_map(context), recipients_format)
        )
        ...
        if attachments_formats:
            ...
            yield compose_email(
                ...
                map(lambda s: Path(s.format_map(context)), attachments_formats),
            )
        else:
...
def get_argparser():
    ...
    parser.add_argument(
        "data_csv", help="CSV file containing addresses and template data"
    )
    ...
    parser.add_argument(
        "-p", "--password-file", help="path to the file containing the SMTP password"
    )
    parser.add_argument(
        "--smtp-host",
        "--host",
    ...
    parser.add_argument(
        "--smtp-port", "--port", help="port of the SMTP server", default=465
    )
